    ...
    <div class="fj-content_view grkh-detail">
        ...
        <div class="fj-block content">
            <div class="fj-block-head kaohe">
				<p class="title fj-fl">地区考核分数记录表（{{deptName}}）</p>
...
window.FjGrkhDetail.prototype = {
    ...
    init:function(listData,vueComp){
        ...
        this.deptName = this.listData[0].deptName
    ...
    setDetailFooterInfo:function(){ //设置汇总信息
        ...
        this.footerResult.text(this.deptName+'：加分'+this.$vueComp.bonusNum+'次，共计'+this.$vueComp.bonusScore+'分。减分'+this.$vueComp.deductNum+'次，共计'+this.$vueComp.deductScore+'分。');
    },
...
window.FjGrkhDetailBlock.prototype={
    ...
    updateWaitState:function(type){  //发送审核通过或拒绝的请求
        ...
        $.ajax({
            url:fjPublic.ajaxUrlDNN + '/updDeptAppraiseLog',
            type:'POST',
            data:{
...
export default {
    ...
    data:function(){
        return {
            breadData:[
                {name:'当前位置:',path:''},
                {name:'考核管理',path:''},
                {name:'地区考核',path:'/local-assessment'},
                {name:'考核明细',path:''}
            ],
            monthVal:fjPublic.date2Month(new Date()), //月份
          deptName:'', //当前的userName
          deptId:'',  //当前的userId
            /*
                状态，0（未审核），1（通过），2（拒绝）
            */
    ...
    beforeRouteEnter:function(to,from,next){ //
        next(function(vm){
          vm.deptId = vm.$route.query.deptId||fjPublic.getLocalData('sub-deptId-detail');
          vm.deptName = vm.$route.query.deptName||fjPublic.getLocalData('deptName-detail');
          //月份根据个人考核列表月份设置
          ...
          fjPublic.setLocalData('sub-deptId-detail',vm.deptId);
          fjPublic.setLocalData('deptName-detail',vm.deptName);
            vm.getUserKhinfo();
        });
    },
    beforeRouteLeave:function(to,from,next){ //
        if(this.oFjGrkhDetail){
            fjPublic.removeLocalData('sub-deptId-detail');
            fjPublic.removeLocalData('deptName-detail');
    ...
    methods:{
        getDetailByKHMonth:function(date){ //切换月份
            ...
            if(selectedTime>curTime){
                this.$message({type:'warning',message:this.monthVal+'月暂无'+this.deptName+'的考核明细数据'});
                return;
            }
            this.getUserKhinfo();
        },
        getUserKhinfo:function(){ //获取个人考核明细数据并添加至页面
            $.when(this.requestDatas()).then(_.bind(function(){
                ...
                if(this.infoListData.length){ //有数据的时候
                    ...
                    this.oFjGrkhDetail = null;
                    this.$message({type:'warning',message:this.monthVal+'月份暂无该地区考核明细信息！'});
                }
        ...
        getPersonAppraiseScore:function(){ //获取个人考核明细数据
			...
			var vm = this;
			$.ajax({
				url:fjPublic.ajaxUrlDNN + '/getDeptAppraiseLogNoPage',
				type:'POST',
				data:{
				            type: '2',
                    month:this.monthVal, //月份，默认当月
                    deptId:this.deptId   //userId
                },
				dataType:'json',
				success:function(data){
                    //console.log(data);
