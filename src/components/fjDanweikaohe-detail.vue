    ...
    <div class="fj-content_view grkh-detail">
        ...
        <div class="fj-block content">
            <div class="fj-block-head kaohe">
				<p class="title fj-fl">单位考核分数记录表（{{deptName}}）</p>
...
export default {
    ...
    data:function(){
        return {
            breadData:[
                {name:'当前位置:',path:''},
                {name:'考核管理',path:''},
                {name:'单位考核',path:'/unit-assessment'},
                {name:'考核明细',path:''}
            ],
    ...
    beforeRouteEnter:function(to,from,next){ //
        next(function(vm){
            vm.deptId = vm.$route.query.deptId||fjPublic.getLocalData('sup-deptId-detail');
            ...
            fjPublic.setLocalData('sup-deptId-detail',vm.deptId);
    ...
    beforeRouteLeave:function(to,from,next){ //
        if(this.oFjGrkhDetail){
            fjPublic.removeLocalData('sup-deptId-detail');
        ...
        getUserKhinfo:function(){ //获取个人考核明细数据并添加至页面
            $.when(this.requestDatas()).then(_.bind(function(){
                ...
                if(this.infoListData.length){ //有数据的时候
                    ...
                    this.oFjGrkhDetail = null;
                    this.$message({type:'warning',message:this.monthVal+'月份暂无该单位考核明细信息！'});
                }
